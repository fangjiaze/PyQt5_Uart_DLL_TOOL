...
class WaveformOscilloscope:
    def __init__(self, filename='lc_waveform.csv', default_points=100):
    ...
    def on_mouse_move(self, event):
        """鼠标移动拖动索引，支持连续平滑拖动 + 追尾自动释放"""
        if self.dragging and event.x is not None and hasattr(self, 'start_mouse_x'):

            # 节流机制：每 50ms 刷新一次
            current_time = time.time()
            if hasattr(self, '_last_move_time') and (current_time - self._last_move_time) < 0.03:
                return
    ...
    def start(self):
        self.ani = animation.FuncAnimation(
            ...
            self.update_plot,
            interval=20,
            blit=False  # 改为 False 也可以直接解决残留问题
        )
        plt.show()